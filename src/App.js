...
function App() {
  ...
  return (
    <div className="font-sans bg-gray-100 text-gray-900 min-h-screen flex flex-col">
            ...
            {(t.team || TEAM).map((member, idx) => (
              <div key={member.name} className="bg-white rounded-xl shadow p-6 flex flex-col items-center border border-gray-100 hover:shadow-lg transition w-full md:w-1/3">
                ...
                {(member.name === 'Ivan Goncharov' || member.name === 'Benjamin Meier-Leeb' || member.name === 'Michele Zanetti') ? (
                  <div className="text-gray-600 text-sm mb-2 text-justify w-full" style={{ minHeight: '210px' }}>
                    {member.bio.split('. ').map((sentence, idx, arr) => {
                      // Only add a dot if it's not the last sentence, or if the original bio does not end with a dot
                      const isLast = idx === arr.length - 1;
                      const needsDot = !isLast || (isLast && !member.bio.trim().endsWith('.'));
                      return (
                        <p key={idx} style={{ textIndent: '2em', marginBottom: '0.5em' }}>
                          {sentence + (needsDot ? '.' : '')}
                        </p>
                      );
                    })}
      ...
      {/* Footer */}
      <footer className="w-full bg-gray-100 border-t border-gray-100 mt-8">
        <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col items-center md:flex-row md:justify-between min-h-[120px] gap-y-4">
          {/* Left: Contact Links */}
          <div className="flex items-center gap-4 justify-center md:justify-start w-full md:w-1/3">
            ...
            <a href="https://www.instagram.com/alfonsoadditive/" target="_blank" rel="noopener noreferrer" className="text-primary hover:no-underline flex items-center mr-4">
              <img src={process.env.PUBLIC_URL + '/instagram.avif'} alt="Instagram" className="h-5 w-5 object-contain" onError={e => { e.target.style.display = 'none'; }} />
              <span>Instagram</span>
            </a>
          </div>
          {/* Center: Contact Info */}
          <div className="text-gray-400 text-sm flex flex-col items-center w-full md:w-1/3 md:items-center">
          ...
          {/* Right: Empty for future use, keeps center info centered */}
        ...
        {showCustomize && (
          <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
            <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full relative">
              ...
              <p className="mb-4 text-sm text-gray-700">Select which types of cookies you want to allow.</p>
